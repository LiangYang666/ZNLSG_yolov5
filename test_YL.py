...
def test(data,
    ...
    if training:  # called by train.py
        ...
        save_dir = Path(increment_path(weights.parent.parent /('test'+f'_{save_name_add}_'), exist_ok=opt.exist_ok))  # increment run
        weights = str(weights)
        # save_dir = Path(save_dir)
        # save_dir = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
        (save_dir / 'labels_pred' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
        (save_dir / 'labels_true' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            # Append to text file
            if save_txt:    # and copy the true txt file
                gn = torch.tensor(shapes[si][0])[[1, 0, 1, 0]]  # normalization gain whwh
                for *xyxy, conf, cls in predn.tolist():
                    xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                    # line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format
                    line = (cls, conf, *xywh) if save_conf else (cls, *xywh)  # label format
                    src = os.path.join(opt.data_dir, 'labels', str(path.stem + '.txt'))
                    dst = os.path.join(str(save_dir / 'labels_true' / (path.stem + '.txt')))
                    shutil.copyfile(src, dst)
                    with open(save_dir / 'labels_pred' / (path.stem + '.txt'), 'a') as f:
    ...
    cmd = 'python v5mAP.py -dir {} -ckpn {}'.format(save_dir, 'best')
    # python mAP.py -dir ../data/bao_data5 -ckpn ckp_bao_3g_6c_l0005_epoch9
    os.system(cmd)
...
if __name__ == '__main__':
    ...
    # python test_YL.py -b 6 --brand Prada --device 1
    # python test_YL.py -b  16 --brand LV --device 1 --save-txt --save-conf
